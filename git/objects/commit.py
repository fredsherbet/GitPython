...
class Commit(base.Object, Iterable, Diffable, Traversable, Serializable):
    ...
    def stats(self):
        ...
        :return: git.Stats"""
        if not self.parents:
            text = self.repo.git.diff_tree(self.hexsha, '--', ignore_all_space=True, minimal=True, numstat=True, root=True)
            text2 = ""
            ...
            text = self.repo.git.diff(self.parents[0].hexsha, self.hexsha, '--', ignore_all_space=True, minimal=True, numstat=True)
